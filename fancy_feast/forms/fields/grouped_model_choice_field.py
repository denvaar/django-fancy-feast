...
class GroupedModelChoiceIterator(ModelChoiceIterator):
    ...
    def __iter__(self):
        ...
        for group, choices in groupby(self.queryset.all(),
            ...
            # If there's a node with no children, just put
            # it inside the "---" optgroup.
            if not group:
                for c in choices:
                    if len(c.get_leafnodes()) == 0:
                        yield ("---", [self.choice(c)])
            else:
...
class GroupedModelMultiChoiceField(Grouped, ModelMultipleChoiceField):
    choices = property(Grouped._get_choices,
        ModelMultipleChoiceField._set_choices)
